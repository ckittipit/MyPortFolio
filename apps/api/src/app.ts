...
export function createApp(contactStore: ContactStore, corsOrigin: string) {
  ...
  app.use((req, res, next) => {
    ...
    const allowExact = requestOrigin ? allowedOrigins.includes(requestOrigin) : false;
    const allowWildcard = requestOrigin
      ? allowedOrigins.some((rule) => rule.startsWith("*.") && requestOrigin.endsWith(rule.slice(1)))
      : false;

    if (requestOrigin && (allowAny || allowExact || allowWildcard)) {
